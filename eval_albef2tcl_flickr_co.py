...
def retrieval_eval(model, ref_model, t_model, t_ref_model, t_test_transform, data_loader, tokenizer, t_tokenizer, device, config):
    ...
    img_attacker = ImageAttacker_Co(config['epsilon'] / 255., preprocess=images_normalize, bounding=(0, 1), cls=args.cls)
    txt_attacker = BertAttackFusion(ref_model, tokenizer, cls=args.cls)
    attacker = MultiModalAttacker(model, img_attacker, txt_attacker, tokenizer, cls=args.cls)
    ...
    for images, texts, texts_ids in tqdm(data_loader):
        ...
        adv_images, adv_texts = attacker.run(images, texts, adv=4, num_iters=config['num_iters'],
                                               alpha=3.0)
...
def eval_asr(model, ref_model, tokenizer, t_model, t_ref_model, t_tokenizer, t_test_transform, data_loader, device, args, config):
    ...
    t_result = itm_eval(t_score_i2t, t_score_t2i, data_loader.dataset.img2txt, data_loader.dataset.txt2img, args.target_model)
...
def main(args, config):
    ...
    test_dataset = pair_dataset(config['test_file'], s_test_transform, config['image_root'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_ckpt', default='./checkpoint/checkpoint_flickr_finetune.pth', type=str)    
    ...
    print(f"Args:\n--batch_size:{args.batch_size}\t--source_model:{args.source_model}\t--target_model:{args.target_model}\n--source_ckpt:{args.source_ckpt}\n--target_ckpt:{args.target_ckpt}")
